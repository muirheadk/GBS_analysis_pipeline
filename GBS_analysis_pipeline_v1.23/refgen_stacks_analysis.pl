...
my ($bwa_input_dir, $pstacks_infile_type, $cstacks_catalog_prefix, $use_existing_catalog, $gbs_sequence_length, $stacks_sql_id, $min_depth_coverage_pstacks, $alpha_value_pstacks, $num_mismatches_tag, $num_threads, $output_dir);
GetOptions(
...
'f=s'    => \$pstacks_infile_type, # The type of alignment file used for input into the pstacks program. Can either be "bam" or "sam".
...
Usage: $0 -i bwa_input_dir -f pstacks_input_file_type -p cstacks_catalog_prefix -e use_existing_catalog -l gbs_sequence_length -b stacks_sql_id -d min_depth_coverage_pstacks -a alpha_value_pstacks -s num_mismatches_tag -c num_threads -o output_dir
