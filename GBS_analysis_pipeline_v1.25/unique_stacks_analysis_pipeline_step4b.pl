...
Usage: $0 -i gbs_fastq_dir -t gbs_fastq_file_type -p cstacks_catalog_prefix -e use_existing_catalog -b stacks_sql_id -d min_depth_coverage_pstacks -m max_nuc_distance_ustacks -n max_align_distance_ustacks -a alpha_value_ustacks -l max_locus_stacks -s num_mismatches_tag -c num_threads -o output_dir
...
sstacks($cstacks_file, $stacks_sql_id, $num_threads, $stacks_output_dir);
...
sub cstacks{
    ...
    if($cstacks_catalog_prefix eq ""){
        ...
        warn "Executing cstacks.....\n\n";
        my $cstacks_joined_soptions = join("\\\n", @cstacks_soptions);
        my $cstacksCmd  = "$cstacks --catalog $cstacks_file -b $stacks_sql_id -o $stacks_output_dir -n $num_mismatches_tag -p $num_threads \\\n $cstacks_joined_soptions 2> $cstacks_log_outfile";
        warn $cstacksCmd . "\n\n";
        system($cstacksCmd) == 0 or die "Error calling $cstacksCmd: $?";
    }
...
sub sstacks{
	...
	# The standard out log file for the cstacks program.
    my $sstacks_log_outfile = join('/', $stacks_log_output_dir, "sstacks.log");
    ...
    my $sstacks_joined_soptions = join("\\\n", @sstacks_soptions);
    warn "Executing sstacks.....\n\n";
    my $sstacksCmd  = "$sstacks -b $stacks_sql_id -c $stacks_catalog_infile \\\n $sstacks_joined_soptions -o $stacks_output_dir -p $num_threads 2> $sstacks_log_outfile";
    warn $sstacksCmd . "\n\n";
